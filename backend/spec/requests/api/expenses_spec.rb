...
RSpec.describe "Api::Expenses", type: :request do
  ...
  describe "GET /api/expenses" do
    let!(:expense1) { Expense.create!(description: "Lunch", amount: 100.00, category: food_category, date: Date.today.prev_month) }
    ...
    it "filters expenses by the year and month of their expense date" do
      get "/api/expenses", params: { year: Date.today.year, month: Date.today.month }

      json = JSON.parse(response.body)
      expect(json.map { |e| e["id"] }).to eq([ expense2.id ])
    end
  end
